...
if __name__ == '__main__':
    ...
    result = [(dataset, ocdt_depth, c_size, c_target,s,ss)
    ...
    for dataset, ocdt_depth, class_size, class_target_size,s,ss in result:
        ...
        for cv_fold, (tr_idx, te_idx) in enumerate(kf.split(features_df)):
            ...
            for bforce, use_hashmaps, use_initial_solution, evaluation_method,prediction_method,prediction_method_leaf in prod:
                ...
                # w arrives from the tree (OCDT is constructed with w=weights below),
                ...
                ocdt_mse_sum_train = mean_squared_error(ysum_train, ysum_pred)
                print(f'OCDT MSE Train: {ocdt_mse_sum_train}')
                ...
                ocdt_mse_sum = mean_squared_error(ysum_test, ysum_pred)
                print(f'OCDT MSE: {ocdt_mse_sum}')
                ...
                perf_df = pd.concat([perf_df, pd.DataFrame({'data': [datasetname], 'fold': [cv_fold], 'depth': [ocdt_depth],
                                                            ...
                                                            'mse gap sum':[(ocdt_mse_sum-dt_mse_sum)/dt_mse_sum], 
                                                            'mse gap':[(ocdt_mse-dt_mse)/dt_mse],
