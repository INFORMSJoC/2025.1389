...
if __name__ == '__main__':
    ...
    dataset_list = ['synthetic_manifold'] # hts, hts_noisy, synthetic_manifold
    evaluation_method_list = ['mse'] # mse, mad, poisson
    prediction_method_leaf_list = ['optimal'] # optimal
    prediction_method_list = ['mean','optimal'] # mean, optimal
    ...
    for c_size, c_target,s,featuresize in (extra_params if (dataset == 'synthetic_manifold' or 
            dataset == 'hts' or dataset == 'hts_noisy') else [(None, None, None)])]
    ...
    for dataset, ocdt_depth, class_size, class_target_size,s,featuresize in result:
        if dataset == 'class':
            ...
            full_df = pd.read_csv(f'{base_folder}/data/hts_noisy_df_size_{class_size}_seed_{s}_v2.csv',dtype='float')
            features_df = full_df[feature_cols]
            numeric_cols = features_df.select_dtypes(include=['number']).columns
            targets_df = full_df[target_cols]
            datasetname = f'hts_noisy_df_size_{class_size}_seed_{s}_v2'            
        ...
        for cv_fold, (tr_idx, te_idx) in enumerate(kf.split(features_df)):
            ...
            if dataset in ['hts','hts_noisy','hts_global']:
            ...
            # dt_mad_train = mean_absolute_error(y_train, y_pred_sklearn)
            print(f'DT MSE_train: {dt_mse_train}')
            # print(f'DT MAD_train: {dt_mad_train}')
            y_pred_sklearn = regressor.predict(X_test)
            dt_mse = mean_squared_error(y_test, y_pred_sklearn)
            # dt_mad = mean_absolute_error(y_test, y_pred_sklearn)
            print(f'DT MSE: {dt_mse}')
            # print(f'DT MAD: {dt_mad}')
            
            
            # `problem` rather than `dataset`: identical for every dataset except
            # hts_noisy, which has no branch of its own and must use the hts
            # constraint set instead of falling through to the default one
            dt_nof_infeasibilities = calculate_number_of_infeasibilities(y_pred_sklearn, X_test, problem, 'DT',
            ...
            perf_df = pd.concat([perf_df, pd.DataFrame({'data': [datasetname], 'fold': [cv_fold], 'depth': [ocdt_depth],
                                                        ...
                                                        # 'mse': [dt_mse], 'Noise free mse': [dt_mse2], 
            ...
            for bforce, use_hashmaps, use_initial_solution, evaluation_method,prediction_method,prediction_method_leaf in prod:
                ...
                nof_infeasibilities_method = lambda y, x: calculate_number_of_infeasibilities(y, x, problem,
                ...
                ocdt_nof_infeasibilities = calculate_number_of_infeasibilities(y_pred, X_test, problem, 'OCDT', ocdt_depth, target_cols)
