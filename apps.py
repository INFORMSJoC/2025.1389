...
def formulate_and_solve_lp_transhipment(y, capacity = None,verbose = False):
    num_targets = len(y)
    ...
    # Create decision variables
    quantity = model.addVars(num_targets, vtype=GRB.CONTINUOUS, lb=0, name="z")
    
    # Create objective function
    sse = gp.quicksum(-y[t] * quantity[t] for t in range(num_targets))
    
    
    model.addConstr(gp.quicksum(-quantity[i] for i in range(num_targets//2)) >= -100, "capacity_constraint_1")
    model.addConstr(gp.quicksum(-quantity[i] for i in range(num_targets//2,num_targets)) >= -10, "capacity_constraint_2")
    ...
    quantities = np.array([quantity[i].X for i in range(num_targets)])
    

    return quantities,-model.objVal

def formulate_and_solve_lp_transhipment_e2e(y, x, w, quantities, capacity, initial_solution, lagrangian_multiplier=0, verbose=False):
    num_instances = y.shape[0]
    num_targets = y.shape[1]
    
    if len(quantities)!=num_instances or len(quantities[0]) != num_targets:
        raise ValueError('Optimization: Index mismatch')
    
    # quantitiesnp = quantities.to_numpy()
    quantitiesnp = quantities
    ...
    # Create decision variables
    quantity_hat = model.addVars(num_targets, vtype=GRB.CONTINUOUS, lb=0, name="z")
    prediction = model.addVars(num_targets, vtype=GRB.CONTINUOUS, name="yhat")
    pi = model.addVars(num_cons, vtype=GRB.CONTINUOUS, lb=0, name="pi")
    regret = {}
    for i in range(num_instances):
        regret[i] = model.addVar(vtype=GRB.CONTINUOUS, lb=0)
            
    # This variable represents (prediction * quantity_hat)
    revenue_term = model.addVars(num_targets, vtype=GRB.CONTINUOUS, lb=0, name="rev_term")

    # Add constraint: revenue_term[t] = prediction[t] * quantity_hat[t]
    for t in range(num_targets):
        model.addConstr(revenue_term[t] == prediction[t] * quantity_hat[t], name=f"link_rev_{t}")

    # Create objective function
    sse = gp.quicksum(gp.quicksum(y[i][t] * quantitiesnp[i][t] - y[i][t] * quantity_hat[t] for t in range(num_targets)) **2
        for i in range(num_instances))
    
    
    # Update the constraint that used the product to use the auxiliary variable as well
    model.addConstr(gp.quicksum(-revenue_term[t] for t in range(num_targets)) <= -100*pi[0] -10 * pi[1], "y_constraint_1")
    
    model.addConstr(gp.quicksum(-quantity_hat[i] for i in range(num_targets//2)) >= -100, "capacity_constraint_1")
    model.addConstr(gp.quicksum(-quantity_hat[i] for i in range(num_targets//2,num_targets)) >= -10, "capacity_constraint_2")
    
    
    for i in range(0, num_targets//2):
        model.addConstr(-pi[0] <= -prediction[i])
    for i in range(num_targets//2,num_targets):
        model.addConstr(-pi[1] <= -prediction[i])
    
    j=0
    for i in range(num_targets//2,num_targets,2):
        model.addConstr(prediction[i-1]-prediction[i] == (j+1)/10, "y_dif_constraint_"+str(j))
        j+=1
    
    model.addConstr(gp.quicksum(prediction[i] for i in range((num_targets//2))) == 1, "y_constraint_1")
    
    # Solve the problem
    model.setObjective(sse, GRB.MINIMIZE)
    model.optimize()
        
    quantities_hat_res = np.array([quantity_hat[i].X for i in range(num_targets)])
    predictions = np.array([prediction[i].X for i in range(num_targets)])
    return predictions, (model.objVal / num_instances) / num_targets, quantities_hat_res
